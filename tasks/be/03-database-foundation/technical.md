...
| Field | Value |
| --- | --- |
| Task ID | `be/03-database-foundation` |
...
DATABASE_HOST, DATABASE_PORT, DATABASE_NAME, DATABASE_USERNAME, DATABASE_PASSWORD, DATABASE_SSL: required; separated values; no implicit defaults; password secret.
...
No business tables. Create only connection/schema/migration foundation.
...
Create/Modify: Modify `apps/api/drizzle.config.ts`, `apps/api/src/database/schema.ts`; create migration/client files only at established database location.
...
- [ ] Database Foundation capability exists at documented boundary.
...
| --- | --- | --- |
| `docs/ARCHITECTURE.md` | repository and layer boundaries | Database Foundation boundary |
| `docs/SECURITY.md` | relevant baseline | security/UI constraints |
| Existing task directory | `be/03-database-foundation` | task identity/order |
