# be/02-environment-validation — Environment Validation

## 1. Metadata

| Field | Value |
| --- | --- |
| Task ID | `be/02-environment-validation` |
...
| Blocks | be/03-database-foundation |
| Execution Order | 2 |
...
- Validate documented environment values with Zod before application startup or CMS mount; reject missing, empty, malformed, and unsupported values without exposing secrets.
- Startup → load process environment → Zod validation → invalid: sanitized error and non-zero exit before app.listen → valid: create app and listen.
...
| --- | --- | --- | --- | --- | --- |
| Variables in `apps/api/.env.example` | Yes | string / integer / boolean / URL | Existing Zod schema: non-empty required values; integer ports; explicit true/false; URL where documented | None — startup fails | Passwords and queue credentials: Yes |
...
Create/Modify: Modify `apps/api/src/config/env.ts`, `apps/api/src/server.ts`, `apps/api/.env.example`; test under `apps/api/tests/`.
...
| Existing task directory | `be/02-environment-validation` | task identity/order |
